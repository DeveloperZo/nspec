...
Claude Code users have dedicated slash commands in `.claude/commands/`:

### Slash commands
- **`/spec <name>`** — Generate a full spec pipeline from the current conversation. Captures the chat context automatically and pipes it to the vibe-to-spec CLI.
...
### How it works
The `/spec` command uses Claude Code's `{{CONVERSATION}}` variable to capture the full chat history, then pipes it to `node bin/nspec.mjs vibe-to-spec <name> --cascade`. No manual transcript file creation needed.

### Example flow
1. Discuss a feature with Claude Code (e.g., "I want to add OAuth login with GitHub")
2. Run `/spec auth-feature`
3. Claude Code pipes the conversation to nSpec, which generates requirements → design → tasks → verify
4. Review the health score and address any gaps
