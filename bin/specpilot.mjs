 ...
 *
 * Env:
 *   SPECPILOT_API_KEY   — OpenAI or Anthropic API key (required for generate/verify/cascade/refine)
 *   SPECPILOT_API_BASE  — Base URL (default: https://api.openai.com/v1)
 *   SPECPILOT_MODEL     — Model ID (default: gpt-4o)
 *   SPECPILOT_SPECS_DIR — Specs folder (default: .specs relative to cwd)
 */
...
const SPECS_DIR  = process.env.SPECPILOT_SPECS_DIR || path.join(process.cwd(), '.specs');
...
function requireApiKey() {
  if (!API_KEY) {
    console.error('Error: SPECPILOT_API_KEY env var is required for this command.');
    process.exit(1);
  }
}
...
async function cmdInit() {
  ...
  if (!name) { console.error('Usage: nspec init <name> [--type bugfix] [--mode design-first] [--template rest-api]'); process.exit(1); }
...
async function cmdGenerate() {
  ...
  if (!name || !stage || !store.ALL_STAGES.includes(stage)) {
    console.error('Usage: nspec generate <name> <stage> [--description "..."]');
...
> This file teaches coding agents (Codex, Claude Code, Cursor, etc.) how to work with the nSpec spec system.
...
Commands:
  ...
  clarify <name> --description "..."  Interactive Q&A before requirements generation
...
Options:
  ...
  --description <text>     Feature description (for requirements / root-cause / clarify)
...
Environment:
  SPECPILOT_API_KEY        API key (required for generate/verify/cascade/refine/clarify)
  SPECPILOT_API_BASE       API base URL (default: https://api.openai.com/v1)
  SPECPILOT_MODEL          Model ID (default: gpt-4o)
  SPECPILOT_SPECS_DIR      Specs folder (default: .specs)`);
